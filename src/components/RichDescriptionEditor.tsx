...
const RichDescriptionEditor: React.FC<RichDescriptionEditorProps> = ({
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      try {
        const finalUrl = await uploadImageToSupabase(file); const newImageBlock: ImageBlock = {
  ...
  const handleImagePropertyChange = (blockId: string, property: 'alt' | 'caption' | 'width', value: string | number) => {
  ...
  const handleConvertBlockType = (blockId: string, newType: 'text' | 'image') => {
    const block = blocks.find(b => b.id === blockId);
    if (!block) return;

    const newBlocks = blocks.map(b => {
      if (b.id === blockId) {
        if (newType === 'text' && b.type === 'image') {
          // Convert image to text (use caption or empty)
          return {
            id: b.id,
            type: 'text' as const,
            content: (b as ImageBlock).caption || ''
          };
        } else if (newType === 'image' && b.type === 'text') {
          // Cannot convert text to image without an actual image
          // This would require opening file picker
          return b;
        }
      }
      return b;
          ...
          <div key={block.id} className="relative group">
            {block.type === 'text' ? (
              <div className="relative">
                ...
                {/* Text Block Convert Option */}
                {(block as TextBlock).content.trim() && (
                  <div className="absolute bottom-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button
                      type="button"
                      onClick={handleAddImage}
                      className="text-xs text-blue-600 hover:text-blue-800 bg-white px-2 py-1 rounded shadow-sm border flex items-center space-x-1"
                      title="Add image after this text"
                    >
                      <ImageIcon size={12} />
                      <span>Add Image</span>
                    </button>
              ...
              <div className="relative" style={{ width: `${(block as ImageBlock).width}%`, margin: '0 auto' }}>
                <div className="relative">
                  <img
                    ...
                    alt={(block as ImageBlock).alt || 'Campaign content'}
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleImagePropertyChange(block.id, 'alt', e.target.value)}
                        placeholder="Describe this image..."
                        className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-primary focus:border-primary"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleImagePropertyChange(block.id, 'caption', e.target.value)}
                        placeholder="Add a caption..."
                        className="w-full px-2 py-1 text-sm border border-gray-300 rounded focus:ring-1 focus:ring-primary focus:border-primary"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Width ({(block as ImageBlock).width}%)</label>
                      <input
                        type="range"
                        min="25"
                        max="100"
                        ...
                        onChange={(e) => handleImagePropertyChange(block.id, 'width', parseInt(e.target.value))}
                        className="w-full"
                      />
                    </div>
                    <div className="flex justify-between items-center pt-1">
                      <button
                        ...
                        title="Convert to text block using caption"
                      >
                        <Type size={12} />
